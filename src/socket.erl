...
-export([open/2, close/1, bind/2, listen/1, listen/2, accept/1, sockname/1, peername/1, recv/1, send/2]).
...
-spec open(Domain :: socket_domain(), Type :: socket_type()) -> {ok, socket()} | {error, Reason :: term()}.
open(Domain, Type) ->
    open_socket(Domain, Type).

-spec close(Socket :: socket()) -> ok.
close(Socket) when is_pid(Socket) ->
    port:call(Socket, close).

-spec bind(Socket :: socket(), Address :: sockaddr() | any | localhost) -> ok | {error, Reason :: term()}.
...
%% @private
open_socket(Domain, Type) ->
    case open_port({spawn, "atomvm_socket"}, [{domain, Domain}, {type, Type}]) of
...
%% @private
validate_sock_addr(loopback) -> loopback;
validate_sock_addr(any) -> any;
...
%% @private
validate_address(loopback) -> loopback;
validate_address(any) -> any;
